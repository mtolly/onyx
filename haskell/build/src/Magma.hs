...
callProcessInQuiet :: FilePath -> FilePath -> [String] -> Action ()
callProcessInQuiet dir = command_
  [ Cwd dir
  , Stdin ""
  , WithStdout True
  , WithStderr True
...
callProcessIn' :: FilePath -> FilePath -> [String] -> Action Bool
callProcessIn' dir prog args = (ExitSuccess ==) <$> command
  [ Cwd dir
  , Stdin ""
  , WithStdout True
  , WithStderr True
...
runMagmaV1 :: FilePath -> FilePath -> Action Bool
...
oggToMogg ogg mogg = withSystemTempDirectory "ogg2mogg" $ \tmp -> do
  ...
  _ <- withExe (callProcessInQuiet tmp) (tmp </> "MagmaCompilerC3.exe") [proj, rba]
