...
midiRB3toGH2
  ...
  -> StackTraceT m (F.Song (GH2File U.Beats), Int)
midiRB3toGH2 song target audio (F.Song tmap mmap onyx) = let
  makeMoods moods = let
...
makeGH2DTA :: SongYaml f -> (Int, Int) -> TargetGH2 -> GH2Audio -> T.Text -> D.SongPackage
makeGH2DTA song preview target audio title = D.SongPackage
  { D.name = title
